...
<div id="manage-dorm">
      ...
      <v-form ref="form" lazy-validation>
        <v-layout wrap>
          <v-flex xs12 sm6 md4 pa-3>
            ...
            <v-select class="shift-left" :items="roomData.room_types" :rules="requiredRules" item-text="name" item-value="id" :label="lang.AddnewRoom.roomType" append-icon="expand_more" :menu-props="{
            offsetY: '',
            transition: 'slide-y-transition',
            bottom: ''
          }" v-model="roomData.room_type_id"></v-select>

            <v-select class="shift-left" :items="roomData.durations" :rules="requiredRules" item-text="name" item-value="id" :label="lang.AddnewRoom.duration" append-icon="expand_more" :menu-props="{
            offsetY: '',
            transition: 'slide-y-transition',
            bottom: ''
          }" v-model="roomData.duration_id"></v-select>

            <v-text-field :label="lang.AddnewRoom.allowedPeople" type="number" :rules="requiredRules" v-model="roomData.people_allowed_number"></v-text-field>

            <v-layout wrap>
              <v-flex xs12 sm8>
                <v-text-field :label="lang.AddnewRoom.price" type="number" :rules="requiredRules" v-model="roomData.price"></v-text-field>
              </v-flex>
              <v-flex xs12 sm4 pl-2>
                <v-select class="shift-left" :rules="requiredRules" :items="roomData.currencies" v-model="roomData.price_currency_id" item-text="code" item-value="id" :label="lang.AddnewRoom.currency" color="success" append-icon="expand_more" :menu-props="{
                  offsetY: '',
                  transition: 'slide-y-transition',
                  bottom: ''
                }"></v-select>
            ...
            <v-text-field :label="lang.AddnewRoom.totalRooms" :hint="lang.AddnewRoom.totalRoomsHint" :rules="requiredRules" type="number" v-model="roomData.total_quota"></v-text-field>
            ...
            <v-autocomplete class="features-input" v-model="roomData.chosen_features" :disabled="isUpdating" :items="roomData.all_features" box chips color="blue-grey lighten-2" :label="lang.AddnewRoom.selectFeatures" item-text="name" item-value="id" multiple>
          ...
          <v-flex xs12 sm6 md4 pa-3>
            ...
            <v-layout row wrap>
              ...
              <template v-else>
                <p class="grey--text">{{lang.AddnewRoom.noPhotos}}</p>
              </template>
            </v-layout>

            <v-layout column>
              <v-flex xs12 class="mt-4">
                <h2 class="heading">{{lang.AddnewRoom.UploadNewHeading}}:</h2>
              </v-flex>
              <v-flex>
          ...
          <v-flex xs12>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn large class="elevation-0" @click="closeEditDialog">{{lang.shared.cancel}}</v-btn>

              <v-btn color="#feae25" large class="elevation-0" :disabled="btnDisabled" :loading="loadingBtn" @click="submitChanges">{{lang.shared.save}}</v-btn>
            </v-card-actions>
          </v-flex>
  ...
  <v-dialog v-model="deletePhoto.confirmDialog" width="500" lazy>
    <v-card>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn class="elevation-0" @click="deletePhoto.confirmDialog = false">{{lang.shared.cancel}}</v-btn>
        <v-btn color="red" class="elevation-0" @click="deleteRoomPhoto" :loading="loadingBtn">{{lang.AddnewRoom.deleteBtn}}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</div>
